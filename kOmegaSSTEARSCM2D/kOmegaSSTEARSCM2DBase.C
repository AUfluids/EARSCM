...
#include "kOmegaSSTEARSCM2DBase.H"
#include "fvOptions.H"
#include "bound.H"
#include "wallDist.H"
...
tmp<volScalarField> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::F1
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::F2() const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::F3() const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::F23() const
{
...
template<class BasicEddyViscosityModel>
void kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::correctNut
(
...
template<class BasicEddyViscosityModel>
tmp<fvVectorMatrix> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::divDevReff
(
    volVectorField& U
) const
{
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::Pk
(
...
kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::epsilonByk
(
...
template<class BasicEddyViscosityModel>
tmp<volScalarField::Internal> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::GbyNu
(
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::kSource() const
{
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::omegaSource() const
{
    return tmp<fvScalarMatrix>
    (
        new fvScalarMatrix
...
template<class BasicEddyViscosityModel>
tmp<fvScalarMatrix> kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::Qsas
(
    const volScalarField::Internal& S2,
...
template<class BasicEddyViscosityModel>
kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::kOmegaSSTEARSCM2DBase
(
    const word& type,
...
:
    ...
    (
        IOobject
        (
            IOobject::groupName("alpha2EARSCM2D", alphaRhoPhi.group()),
            this->runTime_.timeName(),
            this->mesh_,
            IOobject::MUST_READ,
            IOobject::AUTO_WRITE
...
template<class BasicEddyViscosityModel>
void kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::setDecayControl
(
...
template<class BasicEddyViscosityModel>
bool kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::read()
{
...
template<class BasicEddyViscosityModel>
void kOmegaSSTEARSCM2DBase<BasicEddyViscosityModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
...
// New calculations
    ...
    if (EARSCM_.value() == 1.0)
    {
        alpha2EARSCM2D_ = C0I_ 
                 + C1I_*(i1_- (4.13641572e-02))/9.70441569e-03
                 + C2I_*(i2_ - (-4.13023579e-02))/9.75952414e-03;
    }

    if (EARSCM_.value() == 2.0)
    {   
        // scalarList Mean_Funcs(21);

        // std::vector<float> Mean_Funcs = (21, 0.0);

        std::vector<float> Mean_Funcs ={0.0 , 4.13641572e-02, -4.13023579e-02,  2.44245690e-03,
                    -2.53525552e-03, -8.99357060e-04,  1.80516918e-03,
                    1.80113308e-03,  3.66367209e-05, 3.66513910e-05,  
                    9.18447774e-07, -1.80311238e-03,  1.15584151e-04, 
                    -1.19981166e-04, -4.02371071e-05, -1.15568763e-04,  
                    1.19988725e-04, 4.02044463e-05, -3.66150355e-05, 
                    -2.82493804e-06,  2.92861983e-06};

        // scalarList Std_Funcs(21);
        std::vector<float> Std_Funcs ={1.0 , 9.70441569e-03, 9.75952414e-03, 5.53815179e-03, 
                    5.49762408e-03, 3.31065934e-04, 6.26851770e-04, 
                    6.30729609e-04, 1.23420744e-04, 1.23433310e-04, 
                    5.35603343e-07, 6.28708419e-04, 2.66551140e-04, 
                    2.64730219e-04, 1.82322603e-05, 2.66548793e-04, 
                    2.64723111e-04, 1.82599462e-05, 1.23434794e-04, 
                    6.64562481e-06, 6.60783102e-06};

        // scalarList PC1_Coef(21);
        std::vector<float> PC1_Coef =  {0.0 , -0.3293346 ,  0.32952084, -0.07913047,  
                     0.084844  ,  0.32330644, -0.33372914, -0.33320048, 
                     -0.06256836, -0.06265304, -0.27860134, 0.33351261, 
                     -0.08057959,  0.08576212,  0.31675045,  0.08064155, 
                     -0.08580003, -0.31648758,  0.06258588,  0.08063815, 
                     -0.08516232};


        // scalarList PC2_Coef(21);
        std::vector<float> PC2_Coef =  {0.0 , -0.11203638,  0.10852921,  0.30129634, 
                     -0.29668894,  0.08055337, -0.09694584, 
                     -0.09192006,  0.27497115,  0.27494217, 
                     -0.04184519, 0.09443097,  0.30030259, 
                     -0.29574032,  0.06942074, -0.30029092, 
                     0.29577059, -0.06749883, -0.27499029, 
                     -0.29407945,  0.28962333};
    ...
    bijDelta_ = bijDelta_ + ((nut*omega_/(k_ + this->kMin_))*(alpha2EARSCM2D_*T2_) - bijDelta_)*EARSCM_relaxation_;
